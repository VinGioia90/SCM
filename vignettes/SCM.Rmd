...
vignette: >
  ...
  %\VignetteIndexEntry{SCM_vignette}
...
The covariates enter the model through the linear predictor vector $\boldsymbol \eta_i=(\eta_{i1}, \ldots, \eta_{iq})$, with $q= d+d(d+1)/2$, whose components allow specifying parametric and non-parametric components. 
...
<!-- \begin{equation}\boldsymbol \Theta_i=\begin{pmatrix} \eta_{id+1} &  \cdots & \cdots & \cdots& \cdots & \cdots \\ -->
<!--                                \eta_{2d+1} & \eta_{d+2} &  \cdots & \cdots& \cdots  & \eta_{q-d+3} \\ -->

<!--                                \eta_{2d+2} & \eta_{2d+3} & \eta_{d+3}& \cdots   & \cdots & \eta_{q-d+4} \\ -->

<!--                                \vdots & \vdots & \vdots & \ddots   &  \vdots& \vdots \\ -->
<!--                                 \vdots & \vdots & \vdots & \vdots   &  \ddots&  \eta_{q} \\ -->

<!--                              \eta_{q-d+2} & \eta_{q-d+3} & \cdots &  \cdots & \eta_{q} & \eta_{2d}\\ -->

<!-- \end{pmatrix}. \end{equation} -->

<!-- functionally related to the entries of the modified Cholesky decomposition (MCD) of $\boldsymbol \Sigma^{-1}$, that is $\boldsymbol \Sigma^{-1}={\boldsymbol T^\top}{ \boldsymbol D^{-2}} {\boldsymbol T}$, and the  matrix logarithmic (logM) transformation of $\boldsymbol \Sigma$, that is $\log \boldsymbol \Sigma_i$ -->

<!-- The diagonal elements of $\boldsymbol \Theta_i$ correspond to  -->

<!-- - $[\boldsymbol \Theta_i]_{jj} = [\log {\boldsymbol D}^2_i]_{jj}$  for the MCD  -->

<!-- - $[\boldsymbol \Theta_i]_{jj} = [\log \boldsymbol \Sigma_i]_{jj}$ for the logM -->

<!-- for $j=1, \ldots, d$. -->

<!-- The out diagonal elements of $\boldsymbol \Theta_i$ correspond to  -->

<!-- - $[\boldsymbol \Theta_i]_{jk} = [{\boldsymbol T_i}]_{kj}$  of the MCD -->

<!-- - $[\boldsymbol \Theta_i]_{jk} = [\log \boldsymbol \Sigma_i]_{jk}$ of the matrix logarithm of the covariance matrix  -->

<!-- for $j= 2, \ldots, d$  and $k=1, \ldots, j$.  -->

<!-- Smoothing parameter optimization can be carried out via -->

<!-- - generalized Fellner-Schall method (available for both mcd and logm)  -->

<!-- - quasi-Newton optimization (only available for the mcd parametrisation)   -->


<!-- Here, we aim to introduce the most important functions of the package, by means of the  GefCom2014 Dataset. We consider only the hourly loads from 5pm to 8 pm. -->


```{r cleaningData, echo=FALSE, eval=FALSE}
#######################################################################################
# Data Pre processing                                                                 #
# Some notes:                                                                         #
# 1) I removed the 01/01/2005 since there is not information on the midnight          #
# 2) I removed the 01/12/2011 since there is only information on the midnight         #
# 3) There are 3 days with missing observations: 10/03/2006 - 08/27/2011 - 08/28/2011 #
# 4) I removed also 02/02/2005 due to a load 24 missing value                         #
#######################################################################################
# rm(list=ls())
# library(devtools)
# library(electBook)
...
# 
# colnames(GEF14_data)[ 1 : d] <- paste0("load_h", 17 : 20)
# colnames(GEF14_data)[(d + 1) : (2*d)] <- paste0("load24_h", 17 : 20)
# colnames(GEF14_data)[(2 * d + 1) : (3 * d)] <- paste0("temp_h", 17 : 20)
# colnames(GEF14_data)[(3 * d + 1) : (4 * d)] <- paste0("temp95_h", 17 : 20)
# 
...
# 
# # Select the hours: 17 - 18 - 19 -20
# h1_h2 <- 17 : 20
# subs_var <- c(paste0("load_h", h1_h2),
#               paste0("load24_h", h1_h2),
#               paste0("temp_h", h1_h2),
#               paste0("temp95_h", h1_h2),
#              "doy", "dow", "month", "year", "progtime")
# GEF14_d4 <- GEF14_data[, subs_var]
# 
# setwd("C:/Users/Gioia/Desktop/SoftwarePaper/README")
# save(GEF14_d4,  file="GEF14_d4.RData")
```

The multivariate Gaussian additive model is fitted via the `gam_scm()` wrapper
...
* `formula`: list of model formulas provided by the user;
...
# Illustration
We introduce the most important functions of the package by means of the  GefCom2014 dataset (Hong et al., 2016), considering only the hourly loads from 5pm to 8 pm.
...
Note that if you don't specify a model formula for a component of $\Theta$, such element is automatically modelled via intercept, while the model foprmula for a menan vector compnent requires to be specified in any case.
...
The resulting object is of class `"scm"` and can be explored via

- plot of the smooth effects, as in `gam`   
```{r gam plot, echo=TRUE, eval=FALSE, include = TRUE} 
plot(fit1, scale = FALSE, pages = 1) 
```

- summary of linear and smooth effects (`intercept = FALSE` avoids printing the intercepts)  
...
- fitted values in terms of (mean vector, variances, correlations) 

...
get_foo2 <- function ( foo_user2, d) {
  d_len <- nchar(d)
  foo_len <- length(foo_user2)
  foo_bar_lhs <- list()
  foo_bar <-list()
  el_bar <- rep(0, length(foo_user2))
  el_bar2 <- rep(0, length(foo_user2))
  count<-1
  for(j in 1:length(foo_user2)){
    idx_bar <- gregexpr("\\|", foo_user2[[j]])[[2]]
    if(idx_bar[1] != -1){
      el_bar[j] <- j
      idx_bar <- c(0,idx_bar,(nchar( word(as.character(foo_user2[[j]]), 1, sep = "\\~")[2]))+1 )
      for(i in 1:(length(idx_bar)-1)){
        foo_bar_lhs[[count]] <- substring(as.character(foo_user2[[j]])[2], first = idx_bar[i]+1, last = idx_bar[i + 1] -1)
        foo_bar[[count]] <- paste(foo_bar_lhs[[count]], "~", as.character(rhs(foo_user2[[j]]))[2])
        count <- count + 1
      }
    }
  }
  ...
  for(j in (1:length(foo_user2))[-el_bar]){
    for(k in 1 : length(foo_bar)){
      if(lhs.vars(foo_user2[[j]]) ==  lhs.vars(formula(foo_bar[[k]]))){
        foo_bar[[k]] <- paste(foo_bar[[k]],as.character( rhs(foo_user2[[j]]))[2], sep="+")
        el_bar2[j] <- j
      }
    }
  }
  ...
  for(j in (1:length(foo_user2))[-c(el_bar,el_bar2)]){
    if( !(as.character(lhs.vars(foo_user2[[j]])) %in%   name_lhs)) foo_bar[[count]] <- foo_user2[[j]]
    count <- count + 1
  }


  foo_user <- lapply(foo_bar, formula, env = globalenv())
